        ...
        <div style={{ display: 'flex', gap: '10px' }}>
            {['All', 'gray', 'yellow', 'green'].map(status => (
                <button key={status} onClick={() => setStatusFilter(status)} style={{
                    ...
                    }}>{status === 'All' ? 'Все' : STATUS_DB_MAP[status]}</button>
            ))}
        </div>
      </div>
        ...
        <div style={{overflowY: 'auto', flex: 1}}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                ...
                <tbody>
                    ...
                    {filteredClients.length === 0 && <tr><td colSpan="3" style={{padding: '30px', textAlign: 'center', color: '#94a3b8'}}>Клиенты не найдены</td></tr>}
                </tbody>
            </table>
        </div>
      </div>
        ...
        {selectedClient && (
            <>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '25px' }}>
                    {!clientDetails ? <div style={{textAlign:'center', marginTop: '50px', color: '#94a3b8'}}>Загрузка данных...</div> : (
                        <>
                            <div style={{ marginBottom: '30px' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '15px' }}>
                                    <h3 style={{ margin: 0, fontSize: '22px', color: '#1e293b' }}>{clientDetails.name}</h3>
                                    <button onClick={openEditModal} title="Редактировать" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3b82f6' }}><Edit size={20} /></button>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '15px', background: 'white', padding: '15px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                                    <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}><Mail size={16} color="#94a3b8"/> <span style={{color: '#334155'}}>{clientDetails.email || "Email не указан"}</span></div>
                                    <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}><Briefcase size={16} color="#94a3b8"/> <span style={{color: '#334155'}}>{clientDetails.account_number || "Счет не указан"}</span></div>
                                </div>
                            </div>
                            <div style={{ marginBottom: '30px' }}>
                            ...
                            <div>
                                    ...
                                    <div style={{display: 'flex', flexDirection: 'column', gap: '10px'}}>
                                        {clientDetails.files.map((file, idx) => (
                                            <div key={idx} style={{display: 'flex', alignItems: 'center', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '8px', background: '#f9fafb'}}>
                                                <div style={{background: 'white', padding: '8px', borderRadius: '6px', border: '1px solid #e2e8f0', marginRight: '10px'}}><FileText size={20} color="#3b82f6"/></div>
                                ...
                                ) : <div style={{textAlign:'center', padding: '30px', border: '2px dashed #e2e8f0', borderRadius: '8px', color: '#94a3b8', fontSize: '13px'}}>Нет загруженных файлов</div>}
                            </div>
                        </>
                    )}
                </div>
                <div style={{ padding: '20px', borderTop: '1px solid #e2e8f0', background: '#f8fafc', display: 'flex', gap: '10px' }}>
                    <button onClick={() => { window.location.href = `mailto:${clientDetails?.email}` }} className="btn" style={{ flex: 1, background: '#3b82f6', justifyContent: 'center' }} disabled={!clientDetails?.email}><Mail size={18} style={{marginRight: '8px'}}/> Написать</button>
                    <button onClick={handleDeleteClient} className="btn" style={{ background: '#fee2e2', color: '#ef4444', border: '1px solid #fecaca' }}><Trash2 size={18} /></button>
                </div>
            </>
        )}
      </div>
...
const ClientModal = ({ title, data, setData, onSave, onClose }) => (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1100 }}>
        <div className="card" style={{ width: '400px', padding: '30px', position: 'relative' }}>
            <button onClick={onClose} style={{position: 'absolute', top: '10px', right: '10px', background:'none', border:'none', cursor:'pointer'}}><X size={20} /></button>
            <h2 style={{marginTop: 0}}>{title}</h2>
            <div className="input-group"><label>Имя</label><input className="text-input" value={data.name} onChange={e => setData({...data, name: e.target.value})} /></div>
            <div className="input-group"><label>Email</label><input className="text-input" value={data.email} onChange={e => setData({...data, email: e.target.value})} /></div>
            <div className="input-group"><label>Счет</label><input className="text-input" value={data.account} onChange={e => setData({...data, account: e.target.value})} /></div>
            <div className="input-group"><label>Папка</label><input className="text-input" value={data.folder_path} onChange={e => setData({...data, folder_path: e.target.value})} /></div>
