...
import {
  User, Shield, Key, BarChart3, Briefcase,
  Trash2, UserPlus, Check, X, Building2, Edit2, Plus, LogOut
} from 'lucide-react';
...
const ProfilePage = ({ onLogout }) => {
  ...
  const handleCreateUser = async () => {
    ...
    try {
        ...
        fetchUsers(token);
    } catch (err) { toast.error("Ошибка создания"); }
  };
  ...
  const handleAddDept = async () => {
    ...
    try {
        ...
        setNewDeptName("");
        fetchDepartments();
    } catch (err) { toast.error("Ошибка (возможно имя занято)"); }
  };
        ...
        <div style={{ flex: 1, minWidth: '300px' }}>
            <div className="card" style={{ height: '100%' }}>
                ...
                <form onSubmit={handleChangePassword}>
                    <div className="input-group"><label className="input-label">Старый пароль</label><div style={{position: 'relative'}}><Key size={16} style={{position: 'absolute', top: '12px', left: '10px', color: '#94a3b8'}}/><input type="password" className="text-input" style={{paddingLeft: '35px'}} value={oldPass} onChange={e => setOldPass(e.target.value)}/></div></div>
                    <div className="input-group"><label className="input-label">Новый пароль</label><input type="password" className="text-input" value={newPass} onChange={e => setNewPass(e.target.value)}/></div>
                    <div className="input-group"><label className="input-label">Повторите</label><input type="password" className="text-input" value={confirmPass} onChange={e => setConfirmPass(e.target.value)}/></div>
        ...
        <div className="card" style={{ borderTop: '5px solid #dc2626', padding: '0' }}>
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid #e2e8f0' }}>
                <button
                    ...
                    style={{
                        flex: 1, padding: '15px', background: adminTab === 'users' ? 'white' : '#f8fafc',
                        border: 'none', cursor: 'pointer', fontWeight: 600, color: adminTab === 'users' ? '#dc2626' : '#64748b',
                ...
                <button
                    ...
                    style={{
                        flex: 1, padding: '15px', background: adminTab === 'depts' ? 'white' : '#f8fafc',
                        border: 'none', cursor: 'pointer', fontWeight: 600, color: adminTab === 'depts' ? '#dc2626' : '#64748b',
                        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px'
                    }}
                >
            ...
            {adminTab === 'users' && (
                <div style={{ padding: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '15px' }}>
                        <button className="btn" onClick={() => setShowCreateUserModal(true)}><UserPlus size={18} style={{marginRight: '8px'}}/> Добавить</button>
                    </div>
                            ...
                            {users.map(u => (
                                <tr key={u.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                    ...
                                    <td style={{ padding: '10px', textAlign: 'right' }}>
                                        {u.id !== profile.id && (
                                            <button onClick={() => handleDeleteUser(u.id)} style={{background:'none', border:'none', cursor:'pointer', color:'#ef4444'}}><Trash2 size={18}/></button>
                                        )}
                                    </td>
                                </tr>
                            ))}
            ...
            {adminTab === 'depts' && (
                <div style={{ padding: '20px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', background: '#f8fafc', padding: '15px', borderRadius: '8px' }}>
                        ...
                        />
                        <button className="btn" onClick={handleAddDept} disabled={!newDeptName}><Plus size={18}/> Добавить</button>
                    </div>
                                ...
                                {editingDept === d.id ? (
                                    <div style={{display: 'flex', gap: '5px', width: '100%'}}>
                                        <input
                                            ...
                                            style={{padding: '5px', borderRadius: '4px', border: '1px solid #cbd5e1', width: '100%'}}
                                        />
                                        <button onClick={saveDeptName} style={{border:'none', background:'none', cursor:'pointer', color:'#10b981'}}><Check size={18}/></button>
                                    ...
                                    <>
                                        ...
                                        <div style={{display: 'flex', gap: '5px'}}>
                                            <button onClick={() => startEditDept(d)} style={{border:'none', background:'none', cursor:'pointer', color:'#3b82f6'}}><Edit2 size={16}/></button>
                                            <button onClick={() => handleDeleteDept(d.id)} style={{border:'none', background:'none', cursor:'pointer', color:'#ef4444'}}><Trash2 size={16}/></button>
                                        </div>
                                    </>
                                )}
                            </div>
      ...
      {showCreateUserModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 999 }}>
            <div className="card" style={{ width: '400px', padding: '30px' }}>
                ...
                <div className="input-group">
                    <label>Пароль</label>
                    <input className="text-input" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} />
                </div>
                <div className="input-group">
                    <label>Отдел</label>
                ...
                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                    <button className="btn" style={{ background: '#e2e8f0', color: 'black' }} onClick={() => setShowCreateUserModal(false)}>Отмена</button>
                    <button className="btn" onClick={handleCreateUser}>Создать</button>
                </div>
            </div>
        </div>
      )}
