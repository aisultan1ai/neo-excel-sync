...
import {
  ...
  CheckCircle, Clock, Circle, Save, X, Paperclip, FileText, Download, User, AlertTriangle
} from 'lucide-react';
...
const DepartmentsPage = () => {
  ...
  const handleCreateTask = async () => {
    ...
    } catch (err) { toast.error("Ошибка создания задачи"); }
  };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <button
                ...
                style={{
                    ...
                    display: 'flex', alignItems: 'center', gap: '8px', borderBottom: '1px solid #e2e8f0'
                }}>
                    ...
                    <div key={task.id} className="card" style={{ padding: '20px', cursor: 'pointer', border: '1px solid #e2e8f0', borderLeft: `4px solid ${statusInfo.color}`, transition: 'all 0.2s' }} onClick={() => openTask(task)}>
                        <div style={{display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginBottom: '10px'}}>
                            <div style={{color: '#64748b'}}>
                                {activeDept === 'My Tasks' ? (
                                    <>To: <span style={{fontWeight: 600, color: '#3b82f6'}}>{task.to_department}</span></>
                                ) : (
                    ...
                    <div style={{display: 'flex', flexWrap: 'wrap', gap: '10px'}}>
                        ...
                        {attachments.map(file => (
                            <div key={file.id} style={{display: 'flex', alignItems: 'center', gap: '5px', background: '#f1f5f9', padding: '5px 10px', borderRadius: '6px', fontSize: '12px', border: '1px solid #e2e8f0'}}>
                                ...
                                <button onClick={() => openDeleteFileModal(file.id)} style={{border:'none', background:'none', cursor:'pointer'}}><X size={14} color="#ef4444"/></button>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="card" style={{width: '500px', padding: '30px'}}>
                ...
                {activeDept === 'My Tasks' && (
                    <div style={{marginBottom: '15px'}}>
                        <label style={{display: 'block', fontSize: '13px', fontWeight: 600, color: '#64748b', marginBottom: '5px'}}>Получатель (Отдел)</label>
                ...
                <div style={{marginBottom: '15px'}}><input className="text-input" placeholder="Заголовок" value={newTaskTitle} onChange={e => setNewTaskTitle(e.target.value)} /></div>
                <div style={{marginBottom: '15px'}}><textarea className="text-input" placeholder="Описание..." style={{height: '100px', fontFamily: 'inherit'}} value={newTaskDesc} onChange={e => setNewTaskDesc(e.target.value)} /></div>
                <div style={{marginBottom: '20px'}}>
                    <label className="btn" style={{width: '100%', justifyContent: 'center', background: '#f8fafc', border: '1px dashed #cbd5e1', color: '#64748b', cursor: 'pointer'}}>
                        <Paperclip size={16} style={{marginRight: '8px'}}/> {newTaskFile ? newTaskFile.name : "Прикрепить файл (необязательно)"}
      ...
      {showDeleteTaskConfirm && (
        <div style={{
            ...
            <div className="card" style={{ width: '400px', padding: '30px', textAlign: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.2)' }}>
                ...
                <h3 style={{ margin: '0 0 10px 0', fontSize: '20px', color: '#1f2937' }}>Удалить задачу?</h3>
      ...
      {showDeleteFileConfirm && (
        <div style={{
            ...
            <div className="card" style={{ width: '400px', padding: '30px', textAlign: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.2)' }}>
                ...
                <h3 style={{ margin: '0 0 10px 0', fontSize: '20px', color: '#1f2937' }}>Удалить файл?</h3>
                <p style={{ color: '#6b7280', fontSize: '14px', marginBottom: '25px', lineHeight: '1.5' }}>
