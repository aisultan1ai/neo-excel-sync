...
const ReconcileView = () => {
  ...
  const exportTwoFiles = async () => {
    ...
    try {
      const res = await axios.post('/api/tools/excel-reconcile?mode=twofiles&export=1', form, { responseType: 'blob' });
  ...
  const exportDuplicates = async () => {
    ...
    try {
      const res = await axios.post('/api/tools/excel-reconcile?mode=duplicates&export=1', form, { responseType: 'blob' });
          ...
          <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', marginBottom: 16 }}>
            <div style={{ flex: 1, minWidth: 320 }}>
              <SimpleFileBlock
                ...
                selectedCol={instCol1} setSelectedCol={setInstCol1}
              />
            ...
            <div style={{ flex: 1, minWidth: 320 }}>
              <SimpleFileBlock
                ...
                selectedCol={instCol2} setSelectedCol={setInstCol2}
              />
              <div style={{ marginTop: 10 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: '#475569', marginBottom: 6, display: 'block' }}>
                  🧾 Колонка Side (Buy/Sell)
                </label>
                <Select value={sideCol2} onChange={setSideCol2} headers={h2} placeholder="-- Выберите колонку --" />
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
            ...
            <button className="custom-btn" onClick={exportTwoFiles} disabled={loading || !twoRes}
              style={{
                ...
                color: '#0f172a', border: '1px solid #cbd5e1', borderRadius: 12,
              ...
              <div style={{ overflow: 'auto', maxHeight: '55vh' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead style={{ position: 'sticky', top: 0, background: '#f1f5f9' }}>
                    <tr>
                      {['InstrumentKey','Direction','count_file1','count_file2','diff_file1_minus_file2'].map(h => (
        ...
        <>
          <div style={{ maxWidth: 900, margin: '0 auto', width: '100%' }}>
            <SimpleFileBlock
              ...
              selectedCol={paperCol} setSelectedCol={setPaperCol}
            />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
              <div>
                ...
                <Select value={amountCol} onChange={setAmountCol} headers={hd} placeholder="-- Выберите колонку --" />
              </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {dupRes.duplicates_summary?.map((r, i) => (
                        <tr key={i}
                          onClick={() => { setChosen({ PaperKey: r.PaperKey, Amount: r.Amount }); runDuplicates({ PaperKey: r.PaperKey, Amount: r.Amount }); }}
